  ...
  const tableData = quizData.questions.map((questionData) => {
    ...
    if (questionData.structure.kind === "MCQ") {
      ...
      if (
        ...
        correctAnswerIndex < questionData.structure.options.length
      ) {
        answerText = cleanHtml(
          questionData.structure.options[correctAnswerIndex].text
        );
      } else {
        answerText = "Invalid Answer Index";
      }
    } else if (questionData.structure.kind === "BLANK") {
      const correctAnswerIndex = questionData.structure.answer[0];
      if (
        correctAnswerIndex >= 0 &&
        correctAnswerIndex < questionData.structure.options.length
      ) {
      ...
      answerText = "Unknown Question Type";
    }

    // Ensure answerText is not '0'
    if (answerText === "0") {
